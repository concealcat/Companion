...
local locale_cache = {}
function Character:get_localised_item_name(name)
...
function Character:take_item_stack(stack, count, source)
  ...
  --self:say("Took "..given)
  if source and source.valid and given > 0 then
    source.surface.create_entity
    {
      name = "flying-text",
      text = {"", "+", given, " ", self:get_localised_item_name(name), " (", inventory.get_item_count(name), ")"},
...
function Character:give_item(name, count, target)
  ...
  {
    name = "flying-text",
    text = {"", "+", given, " ", self:get_localised_item_name(name), " (", self.entity.get_main_inventory().get_item_count(name), ")"},
