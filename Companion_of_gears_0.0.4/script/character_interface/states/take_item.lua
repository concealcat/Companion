...
function Take_item:update()
  ...
  if not self.character:can_reach_entity(self.target) then
    ...
    self.character:move_to(self.target, self.character:get_reach_distance() - (self.target.get_radius() + 0.5))
    return
  end
  ...
  if count == 0 then
    ...
    game.print({"inventory-restriction.player-inventory-full", self.character:get_localised_item_name(self.name), {"inventory-full-message.main"}})
    return
  end
