...
function Companion.new(player)
  local _ = { player = player, index= player.index }
  setmetatable(_, Companion.metatable)
  script_data.players[player.index] = _
  return _
end
...
function Companion.get_or_make(player_index)
  local _ = script_data.players[player_index]
  if _ and _.player.vaild then return _ end
  _ = game.get_player(player_index)
  if not _ then return end
  return Companion.new(_)
end
...
function Companion:get_or_make_character()
  if self.character and self.character.entity.valid then return self.character end
  self.character = nil
  self:make_character()
  return self.character
end
...
function Companion:on_player_action_command(event)
  local _ = self:get_or_make_character()
  if not _ then return end
  _:clear_state()
  local
